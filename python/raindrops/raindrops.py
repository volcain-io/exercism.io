"""Raindrops exercise from exercism.io python path.

Convert a number into a string that contains raindrop sounds corresponding to
certain potential factors. A factor is a number that evenly divides into
another number, leaving no remainder. The simplest way to test if a one number
...
def convert(number):
    ...
    Convert a number into a string that contains raindrop
    ...
    Returns:
        A string that contains raindrop sounds corresponding
        to certain potential factors. For example:
